...
const syncPublishedCourseEnrollments = async (course) => {
    const matchingStudents = await User.aggregate([
        {
            $match: {
                accountType: 'Student',
    ...
    await Course.findByIdAndUpdate(course._id, {
        $addToSet: {
            studentsEnrolled: { $each: matchingStudentIds },
        },
    })

    await User.updateMany(
        { _id: { $in: matchingStudentIds } },
        {
            $addToSet: {
                courses: course._id,
...
exports.createCourse = async (req, res) => {
    try {
        // extract data
        let { courseName, courseDescription, whatYouWillLearn, category, instructions: _instructions, status, tag: _tag, semester, classSection } = req.body;
        ...
        // validation
        if (!courseName || !courseDescription || !whatYouWillLearn || !semester || !normalizedClassSection
            || !category || !thumbnail || !instructions.length || !tag.length) {
            return res.status(400).json({
                success: false,
                message: 'All Fileds are required (including Semester and Class Section)'
            });
        }
        ...
        const newCourse = await Course.create({
            courseName, courseDescription, instructor: instructorId, whatYouWillLearn, semester: Number(semester), classSection: normalizedClassSection, category: categoryDetails._id,
...
exports.getAllCourses = async (req, res) => {
    try {
        ...
        const allCourses = await Course.find(filter,
            {
                courseName: true, courseDescription: true, thumbnail: true, instructor: true,
                ratingAndReviews: true, studentsEnrolled: true, semester: true, classSection: true
            })
            .populate({
                path: 'instructor',
...
exports.getCourseDetails = async (req, res) => {
    try {
        ...
        const totalDuration = getCourseTotalDuration(courseDetails.courseContent)
...
exports.editCourse = async (req, res) => {
    try {
        ...
        for (const key in updates) {
            if (updates.hasOwnProperty(key)) {
                if (key === "tag" || key === "instructions") {
                    ...
                    course[key] = normalizeSection(updates[key])
                } else {
        ...
        if (course.status === 'Published') {
            await syncPublishedCourseEnrollments(course)
        }
    ...
    try {
        ...
        for (const studentId of studentsEnrolled) {
            await User.findByIdAndUpdate(studentId, {
                $pull: { courses: courseId, enrolledCourses: courseId },
            })
        }
